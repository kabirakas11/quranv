...
import { SURAHS } from './src/data/surahs.ts';
import { SEMANTIC_DOMAINS } from './src/data/semanticDomains.ts';
import { GRAMMAR_TYPES, CURATED_GRAMMAR_DERIVATIONS, normalizeGrammarTypeId } from './src/data/grammarTypes.ts';
...
// Load Fluent Arabic Frequency List (5,155 words from https://fluentarabic.net/quran-frequency-list/)
...
app.get('/api/root/:code', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: `Corpus Quran returned status ${response.status}` });
    }
    ...
    console.error('Error fetching root:', code, error);
    res.status(500).json({ error: `Failed to fetch root ${code}: ${error.message}` });
  }
});
